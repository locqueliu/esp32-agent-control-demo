...
我一直觉得，这类项目最难的地方不是“让模型去调一下硬件”，而是把中间这层关系理顺。模型侧的工具调用该怎么写，桥接层该怎么收口，到了 ESP32 端又该怎么把命令分发给屏幕、灯效、舵机和动作模块，这些东西如果一开始不整理清楚，后面就会越来越乱。
...
因为对我来说，`ESP32 + AI Agent` 真正有意思的不是“能不能调动硬件”，而是中间那层契约到底写得顺不顺：
...
把这层单独整理出来以后，后面不管是继续往设备上扩，还是接别的 Agent 方案，都会轻松很多。
